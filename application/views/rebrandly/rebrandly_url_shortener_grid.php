...
<section class="section section_custom">
	<div class="section-header">
		...
		<div class="section-header-button">
			<a class="btn btn-primary" href="<?php echo base_url("url_shortener/rebrandly");?>">
...
$(document).ready(function($) {
  ...
  var table = $("#mytable").DataTable({
      ...
      ajax: 
      {
        "url": base_url+'url_shortener/rebrandly_shortener_data',
        "type": 'POST',
  	    data: function ( d )
  	    {
 ...
 $(document).on('click','#download_btn',function(event){
 	event.preventDefault();
	 	...
	 	var url = "<?php echo site_url('url_shortener/rebrandly_short_url_download');?>";
    	$("#rebrandly_url_shortener_download_selected").addClass('modal-progress');
	 	...
	 	$.ajax({
	 		...
	 		success:function(response)
	 		{
	 			if (response !="") {
	 				...
	 				$("#rebrandly_url_shortener_download_selected").modal();
	 				$("#total_download_selected").html(response);
          			$("#custom_spinner").html("");
          			$("#rebrandly_url_shortener_download_selected").removeClass('modal-progress');
	 			}
	 			else {
    ...
    .then((willDelete) => {
      if (willDelete) 
      {
        ...
        $.ajax({
          context: this,
          type:'POST' ,
          url:"<?php echo base_url('url_shortener/rebrandly_short_url_delete')?>",
          data:{ids:ids},
          success:function(response){ 
            iziToast.success({title: '',message: '<?php echo $this->lang->line("Your rebrandly shortener data has been deleted successfully."); ?>',position: 'bottomRight'});
            table.draw();
          }
        });
      } 
    ...
    .then((willDelete) => {
      if (willDelete) 
      {
        ...
        $.ajax({
          ...
          data:{ids:ids},
          success:function(response){ 
            iziToast.success({title: '',message: '<?php echo $this->lang->line("Your All rebrandly url shortener data has been deleted successfully."); ?>',position: 'bottomRight'});
            table.draw();
          }
        });
      } 
...
<div class="modal fade show" id="rebrandly_url_shortener_download_selected">
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="background: #fefefe;">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-cut"></i> <?php echo $this->lang->line('Rebrandly URL Shortener'); ?></h5>
