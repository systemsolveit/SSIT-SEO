...
<section class="content-header">
	<h1 class = 'text-info'><i class="fa fa-binoculars"></i> <?php echo $this->lang->line("moz check");?></h1>
</section>
	...
	<div class="row" >
		<div class="col-xs-12">
			<div class="grid_container" style="width:100%; height:720px;">
				...
				url="<?php echo base_url()."rank/moz_rank_data"; ?>" 
				...
				>
					...
					<thead>
						<tr>
							...
							<th field="mozrank_subdomain_normalized" sortable="true">Subdomain Normalized</th>
							<th field="mozrank_subdomain_raw" sortable="true">Subdomain Raw</th>
							<th field="mozrank_url_normalized" sortable="true">URL Normalized</th>
							<th field="mozrank_url_raw" sortable="true">URL Raw</th>
							<th field="http_status_code" sortable="true">HTTP Status Code</th>
							<th field="domain_authority" sortable="true">Domain Authority</th>
							<th field="page_authority" sortable="true">Page Authority</th>
							<th field="external_equity_links" sortable="true">External Equity Links</th>
							<th field="links" sortable="true">Links</th>
			...
			<form class="form-inline" style="margin-top:20px">
				...
				<button class='btn btn-info'  onclick="doSearch(event)"><i class="fa fa-search"></i> <?php echo $this->lang->line("search");?></button> <br/>  <br/>  
				<button type="button" style="width:200px;" class="btn btn-outline-warning download" id = "download_btn"><i class="fa fa-cloud-download"></i> <?php echo $this->lang->line("download selected");?></button>
				<button type="button" style="width:200px;" class="btn btn-outline-warning download" id = "download_btn_all"><i class="fa fa-cloud-download"></i> <?php echo $this->lang->line("download all");?></button>
				<button type="button" style="width:200px;" class="btn btn-outline-danger delete" id = "delete_btn" style = 'margin-bottom:10px'><i class="fa fa-trash"></i> <?php echo $this->lang->line("delete selected");?></button>
				<button type="button" style="width:200px;" class="btn btn-outline-danger delete" id = "delete_btn_all" style = 'margin-bottom:10px'><i class="fa fa-trash"></i> <?php echo $this->lang->line("delete all");?></button>
...
<div id="modal_new_search" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-header">
				...
				<h4 id="new_search_details_title" class="modal-title"><i class="fa fa-binoculars"></i> <?php echo $this->lang->line("moz check");?></h4>
			</div><br/>
				...
				<form enctype="multipart/form-data" method="post" class="form-inline" id="new_search_form" style="margin-bottom:10px">
					...
					<div class="row">						
						<div class="form-group col-xs-12 col-sm-12 col-md-12 col-lg-12">
							<label style="color: <?php echo $THEMECOLORCODE; ?>"><i class="fa fa-globe"></i> <?php echo $this->lang->line("enter URL");?><br/></label>
					...
					<div class="row" style="margin-top: 10px;">						
						<div class="form-group col-xs-12 col-sm-12 col-md-1 col-lg-1">
							<span style="margin-left:5%"><?php echo $this->lang->line("OR"); ?></span>
						</div>
						...
						<div class="form-group col-xs-12 col-sm-12 col-md-7 col-lg-7 clearfix">						
							...
							<button type="button"  id="new_search_button" class="btn btn-primary pull-right"><i class="fa fa-binoculars"></i> <?php echo $this->lang->line("start searching"); ?></button>   
						</div>
					</div>
...
<script>
	...
	$(".download").click(function(){
		...
		var url = "<?php echo site_url('rank/moz_rank_download');?>";
		...
		if(rows == '' && all==0)
		{
			...
			alertify.alert("<?php echo $this->lang->line('alert') ?>","<?php echo $this->lang->line('You have not select any record');?>",function(){});
			return false;
		}
		$.ajax({
			type:'POST',
	...
	function get_bulk_progress()
	{
		var base_url="<?php echo base_url(); ?>";			
		$.ajax({
			url:base_url+'rank/bulk_moz_rank_progress_count',
			type:'POST',
			dataType:'json',
			success:function(response){
				...
				if(width==100) 
				{
					...
					$("#download_div").html('<center><a style="margin: 0px auto;" href="<?php echo base_url()."download/rank/moz_".$this->user_id."_".$this->session->userdata("download_id").".csv"; ?>" target="_blank" class="btn btn-lg btn-warning"><i class="fa fa-cloud-download"></i> <b><?php echo $this->lang->line("download"); ?></b></a></center>');
	...
	$j("document").ready(function(){
		...
		$("#new_search_button").on('click',function(){
			...
			$.ajax({
				url:base_url+'rank/moz_rank_action',
				type:'POST',
				data:{urls:urls},
				success:function(response){		
...
<script>
	$(document).ready(function() {
	    $(".delete").click(function(event) {
	        	...
	        	var result = alertify.confirm('<?php echo $this->lang->line("are you sure that you want to delete this record?") ?>',function(){
	        	...
	        	if(result)
	        	{
	        		...
	        		var base_url="<?php echo base_url(); ?>";		
	        		var url = "<?php echo site_url('rank/moz_rank_delete');?>";
	                var rows = $j("#tt").datagrid("getSelections");
	                var info=JSON.stringify(rows); 
					...
					<div class="row">
						<div class="col-xs-12 col-sm-12 col-md-8 col-md-offset-2 col-lg-8 col-lg-offset-2">
							...
							<?php 
								...
								echo "<a href='".base_url()."download/rank/moz_".$this->user_id."_".$download_id.".csv"."'". "title='Download' class='btn btn-warning btn-lg' style='width:200px;'><i class='fa fa-cloud-download' style='color:white'></i> ".$this->lang->line('download')."</a>";							
							?>
							</div>		
