...
<section class="section">
  <div class="section-header">
    <h1><i class="fab fa-typo3"></i> <?php echo $page_title;?></h1>
    <div class="section-header-breadcrumb">
      <div class="breadcrumb-item"><a href="<?php echo base_url('menu_loader/security_tools'); ?>"><?php echo $this->lang->line("Security Tools"); ?></a></div>
      <div class="breadcrumb-item"><a href="<?php echo base_url('antivirus/scan') ?>"><?php echo $this->lang->line("Malware Scan");?></a></div>
...
<div class="row multi_layout">
      ...
      <form  method="POST" enctype="multipart/form-data"  id="new_search_form">
        ...
        <div class="card-body">
          <div class="form-group">
            <label class="form-label"> <?php echo $this->lang->line("Domain"); ?> <a href="#" data-placement="top" data-toggle="popover" data-trigger="focus" title="<?php echo $this->lang->line("Domain") ?>" data-content='<?php echo $this->lang->line("Put your domain name ") ?>'><i class='fa fa-info-circle'></i> </a></label>
            <textarea id="domain_name" name="domain_name" class="form-control" style="width:100%;min-height: 140px;" rows="10"></textarea>
          </div>
            <div class="form-group">
              <div class="selectgroup selectgroup-pills">
                <label class="selectgroup-item" for="antivirus_google">
                  <input type="checkbox" name="antivirus_google" value="1" id="antivirus_google" class="selectgroup-input" checked>
                  <span class="selectgroup-button"><?php echo $this->lang->line('Google'); ?></span>
                </label>
                <label class="selectgroup-item" for="antivirus_norton">
                  <input type="checkbox" name="antivirus_norton" value="1" id="antivirus_norton" class="selectgroup-input" checked>
                  <span class="selectgroup-button"><?php echo $this->lang->line('Norton'); ?></span>
                </label>
                <label class="selectgroup-item" for="antivirus_mcafee">
                  <input type="checkbox" name="antivirus_mcafee" value="1" id="antivirus_mcafee" class="selectgroup-input" checked>
                  <span class="selectgroup-button"><?php echo $this->lang->line('Mcafee'); ?></span>
                </label>
        ...
        <div class="card-footer bg-whitesmoke mt-66">
            ...
            <button type="button"  id="new_search_button" class="btn btn-primary "><i class="fa fa-search"></i> <?php echo $this->lang->line("Search"); ?></button>
            <button class="btn btn-secondary btn-md float-right" onclick="goBack('antivirus/scan')" type="button"><i class="fa fa-remove"></i> <?php echo $this->lang->line('Cancel'); ?></button>
  ...
  <div class="col-12 col-md-6 col-lg-6 colmid">
    ...
    <div id="middle_column_content" style="background: #ffffff!important;">
      ...
      <div class="card">
        <div class="card-header">
          <h4> <i class="fab fa-typo3"></i> <?php echo $this->lang->line('Malware Scan Results'); ?></h4>
  ...
  $("document").ready(function(){
    ...
    $(document).on('click', '#new_search_button', function(event) {
      event.preventDefault();
        ...
        if($('input[name=antivirus_google]').prop('checked'))
            is_google = 1;
        else
          is_google = 0;

        if($('input[name=antivirus_norton]').prop('checked'))
            is_norton = 1;
        else
          is_norton = 0;

        if($('input[name=antivirus_mcafee]').prop('checked'))
            is_mcafee = 1;
        else
          is_mcafee = 0;
        ...
        if(domain_name==''){
          swal("<?php echo $this->lang->line('Error'); ?>", "<?php echo $this->lang->line('Please enter your domain name first'); ?>", 'error');
          return false;
        }
        ...
        $.ajax({
          ...
          type:'POST',
          data:{domain_name:domain_name,is_google:is_google,is_norton:is_norton,is_mcafee:is_mcafee},
     ...
     var files_list = [];
      $("#file_upload_url").uploadFile({
        url:base_url+"antivirus/read_text_csv_file_antivirus",
        fileName:"myfile",
        ...
        deleteCallback: function (data, pd) {
            var delete_url="<?php echo site_url('antivirus/read_after_delete_csv_txt');?>";
