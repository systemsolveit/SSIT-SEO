...
<section class="section section_custom">
  <div class="section-header">
    ...
    <div class="section-header-button">
     <a class="btn btn-primary" href="<?php echo base_url("antivirus/new_scan");?>">
        <i class="fas fa-plus-circle"></i> <?php echo $this->lang->line("New Scan"); ?>
     </a> 
    </div>
            ...
            <div class="table-responsive2">
                ...
                <thead>
                	<tr>
						...
						<th><?php echo $this->lang->line("Response Code"); ?></th>
						<th><?php echo $this->lang->line("Positives"); ?></th>
						<th><?php echo $this->lang->line("Total Scan"); ?></th>
						<th><?php echo $this->lang->line("Scaned At"); ?></th>
...
$(document).ready(function($) {
  ...
  var table = $("#mytable").DataTable({
      ...
      ajax: 
      {
        "url": base_url+'antivirus/virus_total_scan_data',
        "type": 'POST',
  	    data: function ( d )
  	    {
  ...
  $(document).on('click','.view_report',function(event){
    ...
    $.ajax({
    	url:base_url+'antivirus/virus_total_report',
    	type:'POST',
    	data:{id:id},
 ...
 $(document).on('click','#download_btn',function(event){
 	event.preventDefault();
	 	...
	 	var url = "<?php echo site_url('antivirus/virus_total_scan_download');?>";
	 	$.ajax({
	 		type:'POST',
	 		url:url,
	 		data:{ids:ids},
    ...
    .then((willDelete) => {
      if (willDelete) 
      {
        ...
        $.ajax({
          context: this,
          type:'POST' ,
          url:"<?php echo base_url('antivirus/virus_total_scan_delete')?>",
          data:{ids:ids},
          success:function(response){ 
            iziToast.success({title: '',message: '<?php echo $this->lang->line("Your virus total item has been deleted successfully."); ?>',position: 'bottomRight'});
            table.draw();
          }
        });
      } 
    ...
    .then((willDelete) => {
      if (willDelete) 
      {
        ...
        $.ajax({
          ...
          data:{ids:ids},
          success:function(response){ 
            iziToast.success({title: '',message: '<?php echo $this->lang->line("Your all virus total item has been deleted successfully."); ?>',position: 'bottomRight'});
            table.draw();
          }
        });
      } 
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title text-center"><i class="fas fa-shield-alt"></i> <?php echo $this->lang->line("Report of VirusTotal Scan") ?></h4>
...
<div class="modal fade show" id="virus_total_download_selected">
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="background: #fefefe;">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-shield-alt"></i> <?php echo $this->lang->line('VirusTotal Scan'); ?></h5>
