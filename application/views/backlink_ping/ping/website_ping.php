...
<section class="section section_custom">
  <div class="section-header">
    ...
    <div class="section-header-button">
     <a class="btn btn-primary" href="<?php echo base_url("ping/website_ping");?>">
            ...
            <div class="table-responsive2">
                ...
                <thead>
                	<tr>
        						...
        						<th><?php echo $this->lang->line("Blog Name"); ?></th>      
                    <th><?php echo $this->lang->line("Ping URL"); ?></th>         
        						<th><?php echo $this->lang->line("Blog URL"); ?></th>
        						<th><?php echo $this->lang->line("Blog URL to Ping "); ?></th>
                    <th><?php echo $this->lang->line("Blog RSS Feed URL"); ?></th>
                    <th><?php echo $this->lang->line("Response"); ?></th>
                    <th><?php echo $this->lang->line("Ping at "); ?></th>
...
$(document).ready(function($) {
  ...
  var table = $("#mytable").DataTable({
      ...
      ajax: 
      {
        "url": base_url+'ping/ping_website_data',
        "type": 'POST',
  	    data: function ( d )
  	    {
 ...
 $(document).on('click','#download_btn',function(event){
    ...
    $("#website_ping_dwonload_modal").addClass('modal-progress');
	 	$.ajax({
	 		type:'POST',
	 		url:url,
	 		data:{ids:ids},
	 		success:function(response)
	 		{
	 			if (response !="") {
	 				...
	 				$("#website_ping_dwonload_modal").modal();
	 				$("#total_download_selected").html(response);
          $("#custom_spinner").html("");
          $("#website_ping_dwonload_modal").removeClass('modal-progress');
	 			}
	 			else {
    ...
    .then((willDelete) => {
      if (willDelete) 
      {
        ...
        $.ajax({
          context: this,
          type:'POST' ,
          url:"<?php echo base_url('ping/ping_website_delete')?>",
          data:{ids:ids},
          success:function(response){ 
            iziToast.success({title: '',message: '<?php echo $this->lang->line("Your website ping data has been deleted successfully."); ?>',position: 'bottomRight'});
            table.draw();
          }
        });
      } 
    ...
    .then((willDelete) => {
      if (willDelete) 
      {
        ...
        $.ajax({
          ...
          data:{ids:ids},
          success:function(response){ 
            iziToast.success({title: '',message: '<?php echo $this->lang->line("Your all website ping data has been deleted successfully."); ?>',position: 'bottomRight'});
            table.draw();
          }
        });
      } 
...
<div class="modal fade show" id="website_ping_dwonload_modal">
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="background: #fefefe;">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-link"></i> <?php echo $this->lang->line('Website Ping'); ?></h5>
