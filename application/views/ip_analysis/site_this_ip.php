...
<section class="section section_custom">
  <div class="section-header">
    ...
    <div class="section-header-button">
     <a class="btn btn-primary" href="<?php echo base_url("ip/sites_same_ip");?>">
...
$(document).ready(function($) {
  ...
  var table = $("#mytable").DataTable({
      ...
      ajax: 
      {
        "url": base_url+'ip/site_this_ip_data',
        "type": 'POST',
  	    data: function ( d )
  	    {
    ...
    .then((willDelete) => {
      if (willDelete) 
      {
        ...
        $.ajax({
          context: this,
          type:'POST' ,
          url:"<?php echo base_url('ip/site_this_ip_delete')?>",
          data:{ids:ids},
          success:function(response){ 
            iziToast.success({title: '',message: '<?php echo $this->lang->line("Your sites in same ip data has been deleted successfully."); ?>',position: 'bottomRight'});
            table.draw();
          }
        });
      } 
    ...
    .then((willDelete) => {
      if (willDelete) 
      {
        ...
        $.ajax({
          ...
          data:{ids:ids},
          success:function(response){ 
            iziToast.success({title: '',message: '<?php echo $this->lang->line("Your all sites in same ip data has been deleted successfully."); ?>',position: 'bottomRight'});
            table.draw();
          }
        });
      } 
...
<div class="modal fade show" id="site_in_same_ip_download_selected">
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="background: #fefefe;">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-map-marker-alt"></i> <?php echo $this->lang->line('Sites In Same IP'); ?></h5>
