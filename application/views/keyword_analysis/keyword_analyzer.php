...
<div class="row multi_layout">
      ...
      <form  method="POST" enctype="multipart/form-data"  id="new_search_form">
        ...
        <div class="card-body">
          <div class="form-group">
            <label class="form-label"> <?php echo $this->lang->line("Keyword Analyzer"); ?> <code>*</code></label>
            <input id="keyword_domain_name" name="keyword_domain_name" class="form-control" placeholder="<?php echo $this->lang->line('Domain Name'); ?>"  />
          </div>
        </div>

        <div class="card-footer bg-whitesmoke mt-232">

            <button type="button"  id="new_search_button" class="btn btn-primary "> <?php echo $this->lang->line("Analyze"); ?></button>
            <button class="btn btn-secondary btn-md float-right" onclick="goBack('keyword/keyword_analyzer')" type="button"><i class="fa fa-remove"></i> <?php echo $this->lang->line('Cancel'); ?></button>
  ...
  <div class="col-12 col-md-8 col-lg-8 colmid">
    ...
    <div id="middle_column_content" style="background: #ffffff!important;padding: 20px;">
      ...
      <div class="card">
        <div class="card-header">
          <h4> <i class="fas fa-tags"></i> <?php echo $this->lang->line('Keyword Analyzer Results'); ?></h4>
    ...
    $(document).on('click', '#new_search_button', function(event) {
      event.preventDefault();
        ...
        if(keyword_domain_name==''){
          swal("<?php echo $this->lang->line('Error'); ?>", "<?php echo $this->lang->line('Please enter your domain name'); ?>", 'error');
          return false;
        }        
        ...
        $.ajax({
          url:base_url+'keyword/keyword_analyzer_data',
          type:'POST',
