...
<section class="section section_custom">
  <div class="section-header">
    ...
    <div class="section-header-button">
     <a class="btn btn-primary" href="<?php echo base_url("keyword/auto_keyword");?>">
...
$(document).ready(function($) {
  ...
  var table = $("#mytable").DataTable({
      ...
      ajax: 
      {
        "url": base_url+'keyword/keyword_suggestion_data',
        "type": 'POST',
  	    data: function ( d )
  	    {
	 	...
	 	$.ajax({
	 		...
	 		success:function(response)
	 		{
	 			if (response !="") {
	 				...
	 				$("#keyword_sugesstion_download_selected").modal();
    ...
    .then((willDelete) => {
      if (willDelete) 
      {
        ...
        $.ajax({
          context: this,
          type:'POST' ,
          url:"<?php echo base_url('keyword/keyword_suggestion_delete')?>",
          data:{ids:ids},
          success:function(response){ 
            iziToast.success({title: '',message: '<?php echo $this->lang->line("Your keyword auto suggestion data has been deleted successfully."); ?>',position: 'bottomRight'});
            table.draw();
          }
        });
      } 
    ...
    .then((willDelete) => {
      if (willDelete) 
      {
        ...
        $.ajax({
          ...
          data:{ids:ids},
          success:function(response){ 
            iziToast.success({title: '',message: '<?php echo $this->lang->line("Your all keyword auto suggestion data has been deleted successfully."); ?>',position: 'bottomRight'});
            table.draw();
          }
        });
      } 
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="background: #fefefe;">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-tag"></i> <?php echo $this->lang->line('Keyword Suggestion'); ?></h5>
