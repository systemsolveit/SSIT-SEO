		...
		if(!empty($unique_fields))
		{
			...
			foreach($edit_fields as $edit_field)
			{
				...
				if(in_array( $field_name, $unique_fields) )
				{
					...
					$field_name_value = $this->ci->input->post('field_name',true);
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	// re written action after delete to show success message from message.php and to implement alertify
	protected function delete_layout($delete_result = true)
	{
		$ci = &get_instance();
		if($delete_result === false)$ci->session->set_flashdata('delete_error_message',1); 
		else $ci->session->set_flashdata('delete_success_message',1); 
	...
	public function getStateInfo()
	{
		...
		switch ($state_code) {
			...
			case 17: //print
				...
				{
					$state_info->per_page = is_numeric($_POST['per_page']) ? $ci->input->post('per_page',true) : null;
				}
				if(!empty($_POST['page']))
				{
					$state_info->page = is_numeric($_POST['page']) ? $ci->input->post('page',true) : null;
				}
				...
				{
					$state_info->order_by = $ci->input->post('order_by',true);
				}
				if(!empty($_POST['search_text']))
				{
					if(empty($_POST['search_field']))
					{
						...
						$search_text = strip_tags($ci->input->post('search_field',true));

						$state_info->search = (object)array( 'field' => null , 'text' => trim($ci->input->post('search_text',true)) );

					}
					else
					{
						$state_info->search	= (object)array( 'field' => strip_tags($ci->input->post('search_field',true)) , 'text' => trim($ci->input->post('search_text',true)) );
					}
				}
			break;
			...
			case 13:
				$state_info->field_name = $ci->input->post('field_name',true);
				$state_info->search 	= $ci->input->post('term',true);
			break;
